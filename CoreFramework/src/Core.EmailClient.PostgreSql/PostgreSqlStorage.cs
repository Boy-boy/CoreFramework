...
{
    internal class PostgreSqlStorage : IEmailStorage
    {
        private readonly ILogger<PostgreSqlStorage> _logger;
        private readonly PostgreSqlEmailStorageOptions _options;

        public PostgreSqlStorage(IOptions<PostgreSqlEmailStorageOptions> options,
        ...
        public async Task InitializeAsync(CancellationToken cancellationToken = default)
        {
            ...
            var sql = $@"
CREATE SCHEMA IF NOT EXISTS {_options.DbSchema};
  ...
  PRIMARY KEY (Id)
        ...
        public async Task<string> AddAsync(AddEmailModel model, object dbTransaction = null, CancellationToken cancellationToken = default)
        {
            ...
            object[] sqlParams =
            {
                ...
                new NpgsqlParameter("@Sender", model.Sender),
                new NpgsqlParameter("@SenderAddress", model.SenderAddress),
                new NpgsqlParameter("@Recipients",NpgsqlDbType.Jsonb){Value=model.Recipients},
                new NpgsqlParameter("@Cc",NpgsqlDbType.Jsonb){Value=model.Cc??new List<string>()},
                new NpgsqlParameter("@Bcc",NpgsqlDbType.Jsonb ){Value=model.Bcc??new List<string>()},
                new NpgsqlParameter("@Subject", model.Subject),
                ...
                new NpgsqlParameter("@CreatorId", model.CreatorId),
...
WHERE Id=@Id";
        ...
        public async Task<List<EmailMessage>> GetAsync(QueryEmailModel query, CancellationToken cancellationToken = default)
        {
            ...
            if (query.IsSend.HasValue)
            {
                ...
                sqlParams.Add(new NpgsqlParameter("@IsSend", query.IsSend.Value));
            }
        ...
        {
            return $"{_options.DbSchema}.{_options.DbTable}";
        }
    }
}
