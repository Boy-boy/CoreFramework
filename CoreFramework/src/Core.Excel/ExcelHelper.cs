...
namespace Core.Excel
{
    public static class ExcelHelper
    {
        ...
        {
            return GetExcelStream(sources, new Dictionary<string, IList<ExcelColumn>>()
            {
                { sheetName, columns }
            });
        }
        ...
        /// <summary>
        /// 传入数据，返回Excel流文件，支持多个sheet
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="sources">数据源</param>
        /// <param name="sheets">sheet信息，包括名称和列定义</param>
        /// <returns></returns>
        public static MemoryStream GetExcelStream<T>(this IList<T> sources, IDictionary<string, IList<ExcelColumn>> sheets)
        {
            ...
            foreach (var sheet in sheets)
            {
                ...
                for (var row = 1; row <= sources.Count + 1; row++)
                {
                    ...
                    for (var cl = 1; cl <= columns.Count; cl++)
                    {
                        if (row == 1) //设置表头
                        {
                            var excelCellStyle = columns[cl - 1].HeadCellStyle;
                            excelCellStyle?.ConfigCellStyle(worksheet.Cells[row, index]);

                            worksheet.Cells[row, index].Value = columns[cl - 1].DisplayName;
                        }
                        else
                        {
                            var excelCellStyle = columns[cl - 1].ContentCellStyle;
                            excelCellStyle?.ConfigCellStyle(worksheet.Cells[row, index]);
                ...
                //设置Excel列宽
                worksheet.Cells.AutoFitColumns(1.5);
            }
