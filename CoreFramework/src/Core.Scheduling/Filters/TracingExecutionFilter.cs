...
{
    /// <summary>
    /// 分布式追踪过滤器:为每次 handler 执行创建 <see cref="Activity"/>,
    /// 订阅 <see cref="SchedulingDiagnostics.ActivitySourceName"/> 即可让 OpenTelemetry 拿到 span。
    /// 受 <see cref="SchedulingFilterOptions.EnableTracing"/> 控制,关时不创建 Activity。
    /// </summary>
    internal sealed class TracingExecutionFilter : IHandlerExecutionFilter, IDisposable
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            // 用 Unix ms (long) 代替 ISO 8601 字符串,省一次字符串分配;OTel/Jaeger 均识别 long
